...
// 两节点之间关键路径条数，可能有多条，皆为shortestPath[path]数组的元素
...
// 找到对应节点之间关键路径中的最短的路径长度，并将选中的核心节点子图保存
...
onMounted(() => {
    ...
    // HIN 原本的项目是监听选择平行坐标选中的节点变化，这里直接模拟数据
    clearSvg()
    initChart()
    // HIN
})
...
function initChart() {
    ...
    colorToolBox.attr('transform', `translate(-60,0)`)
...
<template>
    <div id="rectangle-chart">
        <div class="chart-header">矩阵链路</div>
